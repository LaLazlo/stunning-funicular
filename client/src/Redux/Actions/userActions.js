...
const getAssignedMejsAction = () => async(dispatch, getState) =>{
    try {
        dispatch({ type: userConstants.GET_ASSIGNED_MEJ_REQUEST });
        const response = await userApi.getAssignedMejsService(tokenProtection(getState));
        dispatch({ type: userConstants.GET_ASSIGNED_MEJ_SUCCESS, payload: response });
    } catch (error) {
        ErrorsAction(error, dispatch, userConstants.GET_ASSIGNED_MEJ_FAIL);

    }
};


const deleteAssignedMejsAction = () => async(dispatch, getState) =>{
    try {
        dispatch({ type: userConstants.DELETE_ASSIGNED_MEJ_REQUEST });
        await userApi.deleteAssignedMejsService(tokenProtection(getState));
        dispatch({ type: userConstants.DELETE_ASSIGNED_MEJ_SUCCESS });
        toast.success("MEJ attribué supprimé")
    } catch (error) {
        ErrorsAction(error, dispatch, userConstants.DELETE_ASSIGNED_MEJ_FAIL);
...
export {
    ...
    getAssignedMejsAction,
    deleteAssignedMejsAction,
    getAllUsersAction,
    deleteUserAction,
};